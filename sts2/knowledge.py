...
from sts2.config import DATA_DIR, DUAL_SUFFIXES, MODS_DIR
...
class KnowledgeBase:
    ...
    @staticmethod
    def _drop_dual_aliases(rows: list[dict]) -> list[dict]:
        """Drop CARD.X_EVENT / _QUEST / _TOKEN when CARD.X is also present.

        The wiki gives a card's event, quest and token appearances their own
        pages, so the scrape lands them as separate ids with the same name and
        text — two Lantern Keys, two Spoils Maps, two Apotheoses. The game has
        no such ids: every one of them is absent from its own card table while
        every un-suffixed base is there, and no save file has ever referenced
        one, so nothing can key off them. localize.py already treats them as
        aliases (DUAL_SUFFIXES -> resolve_key); this is the same call, made
        where the card list is built. Filtered on load rather than edited out
        of cards.json because that file is regenerated by the fetcher.

        A suffixed id with no un-suffixed twin is the card's only row and stays.
        """
        ...
        for r in rows:
            ...
            alias = next((f"CARD.{stem[:-len(s)]}" for s in DUAL_SUFFIXES
        ...
        if dropped:
            log.info("Cards: dropped %d duplicate event/quest/token aliases", dropped)
        return kept

    def _load_all(self):
        card_rows = self._load_json("cards.json")
        for d in self._drop_dual_aliases(card_rows):
            try:
    ...
    def get_cards(self, character: str = None, card_type: str = None,
        ...
        if rarity:
            # Comma-separated so the UI can offer a grouped filter ("Other" is
            # Curse,Status,Quest). A single value behaves exactly as before.
            wanted = {r.strip().lower() for r in rarity.split(",") if r.strip()}
            result = [c for c in result if c.rarity.lower() in wanted]
        if cost:
