...
The Cinzel typeface is © Natanael Gama. Kreon is © Julia Petretta. Both are
under the SIL Open Font License 1.1, whose text is bundled at
`sts2/static/fonts/OFL.txt`.

Kreon is the face Slay the Spire 2 itself sets card text in. It is fetched from
Google Fonts rather than extracted from the game, so what ships here is the
upstream open-licensed release, not a game asset.
